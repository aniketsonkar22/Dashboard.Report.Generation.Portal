...
import { DntService } from 'src/app/services/dnt-service';
...
export class DntComponent implements OnInit {
  ...
  // DnT Configuration
  kpiLabel = 'Drills & Tests Conducted';
  displayedColumns = ['date', 'sector', 'drillType', 'drillConducted', 'plannedDrills', 'correctiveActions', 'correctiveType', 'createdBy', 'actions'];
  ...
  fetchReports(): void {
    ...
    this.dntService.getReports(params).subscribe({
      next: (response: any) => {
        let items: any[] = [];
        let totalCount = 0;
  ...
  saveEdit(item: BaseKpiItem): void {
    ...
    this.dntService.updateReport(item.id, updated).subscribe({
      next: (response) => {
  ...
  private saveNewKpiRecord(payload: any): void {
    ...
    this.dntService.createReport(payload).subscribe({
      next: (response) => {
  ...
  saveNewRecord(): void {
    const payload: any = {
      ...
      drillType: this.newRecordData.drillType,
      drillConducted: Number(this.newRecordData.drillConducted),
      plannedDrills: Number(this.newRecordData.plannedDrills),
      correctiveActions: Number(this.newRecordData.correctiveActions),
      correctiveType: this.newRecordData.correctiveType
    };
